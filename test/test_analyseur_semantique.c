...
void test_controler_jointures_succes() {
    ...
    t_jointure elt1 = { .cible = malloc(sizeof(char) * (strlen("user_lines") + 1)), .condition = malloc(
            sizeof(char) * (strlen("users.id=user_lines.user_id") + 1)), };
    t_jointure elt2 = { .cible = malloc(sizeof(char) * (strlen("lines") + 1)), .condition = malloc(
            sizeof(char) * (strlen("lines.id=user_lines.line_id") + 1)), };
    ...
    liste[0] = elt1;
    liste[1] = elt2;
    t_jointures jointures = { .nb_jointures = 2, .liste = liste };
    t_requete requete = { .cible = "users", .jointures = jointures, };
...
void test_controler_jointures_pas_de_prefixe() {
    ...
    t_jointure elt1 = { .cible = malloc(sizeof(char) * (strlen("user_lines") + 1)), .condition = malloc(
            sizeof(char) * (strlen("id=user_id") + 1)), };
    strcpy(elt1.cible, "user_lines");
    strcpy(elt1.condition, "id=user_id");
    liste[0] = elt1;
    t_jointures jointures = { .nb_jointures = 1, .liste = liste };
    t_requete requete = { .cible = "users", .jointures = jointures, };
...
void test_controler_jointures_cible_non_declaree() {
    t_jointure* liste = malloc(sizeof(t_jointure));
    t_jointure elt1 = { .cible = malloc(sizeof(char) * (strlen("user_lines") + 1)), .condition = malloc(
            sizeof(char) * (strlen("users.id=user_mylines.user_id") + 1)), };
    strcpy(elt1.cible, "user_lines");
    ...
    t_jointure elt2 = { .cible = malloc(sizeof(char) * (strlen("user_lines") + 1)), .condition = malloc(
            sizeof(char) * (strlen("myusers.id=user_lines.user_id") + 1)), };
...
void test_controler_conditions_succes() {
    t_condition fg = { .valeur = "id=20", .fils_gauche = NULL, .fils_droit = NULL };
    t_condition fd = { .valeur = "name=Robert", .fils_gauche = NULL, .fils_droit = NULL };
    t_condition condition = { .valeur = "and", .fils_gauche = &fg, .fils_droit = &fd };
    t_jointures jointures = { .nb_jointures = 0 };
    t_requete requete = { .cible = "users", .condition = condition, .jointures = jointures };
...
void test_controler_conditions_pas_de_prefixe() {
    t_condition fg = { .fils_gauche = NULL, .fils_droit = NULL };
    fg.valeur = malloc(sizeof(char) * (strlen("id=20") + 1));
    strcpy(fg.valeur, "id=20");
    t_condition fd = { .fils_gauche = NULL, .fils_droit = NULL };
...
void test_controler_projections_succes() {
    ...
    tableau[0] = "id";
    tableau[1] = "name";
    t_projection projection = { .taille = 2, .champs = tableau };
    t_jointures jointures = { .nb_jointures = 0 };
    t_requete requete = { .cible = "users", .projection = projection, .jointures = jointures };
